...
Keywords: no slop, no-slop check, sweep the repo, clean up the repo, tidy the repo, is this clean enough to ship, clean before shipping, check for slop, fix the slop, audit the capability layer, audit the skill layer, review the .claude folder, is the skill layer bloated, are the skills clean, are the skills overlapping, did the layer rot, clean up the skills, clean up the claude folder, skill layer health, layer decay, architectural decay, stray todos, leftover placeholders
