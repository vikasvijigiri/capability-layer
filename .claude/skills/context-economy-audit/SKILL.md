...
Audits this repo's `.claude/` layer (skills, hooks, agents, capabilities,
...
- "reduce token usage" / "second brain" mentioned → the `.claude/` config layer
  (skills, hooks, agents, capability indexes, `CLAUDE.md`).
...
paid on every single turn: the `UserPromptSubmit` hook chain, and the number of
discoverable skill descriptions under `.claude/skills/`. The 46 capability
skills under `.claude/capabilities/*/skills/` cost nothing until loaded — that
is the point of keeping them out of `.claude/skills/`, and a finding that
proposes promoting them must price that in.
