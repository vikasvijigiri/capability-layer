---
name: architecture
description: Generate and compare genuinely different approaches, then choose one. Also decides a user-facing surface's visual and interaction rules, writing them into DESIGN.md. Covers architecture, build versus buy, framework choice, rollout strategy, and any screen or flow a person looks at. Triggers include "any ideas", "let us brainstorm", "what are our options", "how should we approach this", "should I use X or Y", "compare these two designs", "I am stuck", "design this screen", "what should this look like", "make this look better", "set up a design system", "pick the colors". Use this whenever the approach is open or a surface has no design contract, even if not asked. Do NOT use once a design exists, when only scope is vague (task-analysis), or to implement the choice.
...
the solution space, keep generation separate from judgement, choose a
direction and say why, then commit it to `docs/specs/`.

**Design contract**: read `references/design-contract.md` when the spec
...
states, and the accessibility floor. Its own output is `DESIGN.md`, not
`docs/specs/`.

Cap visible output at ~500 tokens per turn. Design sections are the
exception — scale those to complexity, up to 200-300 words each.

<HARD-GATE>
Do NOT write code, scaffold a project, or invoke an implementation skill from
here. This skill produces a spec (or `DESIGN.md`) and stops. Every project,
regardless of perceived simplicity.

**Open no dialogue at all. Never call `AskUserQuestion` from this skill.** The
chain has exactly two places that stop and wait for a person — the finished
plan at the end of `task-analysis`, and the shipment approval in `release-git`.
This is neither.
...
belongs. `task-analysis` collects every marker in the repository and resolves them
...
the wedge, constraints, how success is measured. **A user-facing surface** uses
`references/design-contract.md`'s sections instead, writing `DESIGN.md`.
...
**10. Hand off** to `task-analysis`. Nothing else.

## Design guidance (technical isolation and clarity)
...
digraph architecture {
    ...
    "Present the whole design\n(spec or DESIGN.md)" [shape=box];
    "Write + commit the artefact" [shape=box];
    "Self-review\n(fix inline)" [shape=box];
    "Invoke task-analysis" [shape=doublecircle];
    ...
    "Present the whole design\n(spec or DESIGN.md)" -> "Write + commit the artefact";
    "Write + commit the artefact" -> "Self-review\n(fix inline)";
    "Self-review\n(fix inline)" -> "Invoke task-analysis";
    "Invoke task-analysis" -> "Gate 1: every marker,\none question" [style=dashed];
}
```

The dashed edge leaves this skill. Gate 1 belongs to `task-analysis`, and it is
...
- Invoking anything but `task-analysis` at the end.
...
**The terminal state is invoking `task-analysis`.** Once the spec (or
`DESIGN.md`) is written and committed, invoke it in the same turn — do not
wait for the user to read it first. Do not invoke any other skill, and do not
start implementing: the spec says what to build, never in what order or how
each piece is proved.

## Routing

- Mandatory validator: none — this produces a spec or `DESIGN.md`, not a
  change to running code. Step 8's self-review is the only check here; the
  human checkpoint is Gate 1, in `task-analysis`, and it is not this skill's
  to hold.
- Terminal handoff: `task-analysis`.
- **Dispatched by `task-analysis`, and it returns there.** Not a stage
  entered beside stage 1: there is one door, and stage 1 calls this skill
  when Goal or Outputs cannot be filled because the approach is open, or
  when the work touches a user-facing surface with no design contract. It
  must run *before* the brief is written — a finished brief already commits
  to a solution, which would reduce this to variations on an answer already
  given.
- Use `refactoring` after implementation for token, state, accessibility,
  and anti-pattern checks against a written `DESIGN.md`; do not duplicate
  its repository-wide sweep here.
...
its answer belongs, and the spec or `DESIGN.md` is written and committed — with
nobody having been interrupted to get there.
