# Task log — through 2026-08-31

Retired `TASK.md` content, moved here on 2026-09-01 when `TASK.md` became a
capped <=6-row ledger (`decisions/2026-09-01-knowledge-doc-head-contract.md`).
Two parts: the entries that were still sitting in `## Active` at migration
time (every one already at a terminal status — merged, delivered, or closed —
but never moved), then the original append-only `## Completed` archive.
Nothing here is live. The current ledger is `TASK.md`; delivery history is
`LOG.md` and the PRs.

---

## Former `## Active` entries (all terminal at 2026-09-01)
...
### 2026-08-24 — UI/UX design (web + iOS/Android) MCP wiring
- **Goal:** extend `architecture`'s design-contract mode with iOS (Apple HIG) and
  Android (Material Design 3) platform guidance and wire Figma MCP tools into it,
  instead of new top-level skills — per `docs/research/2026-08-23-ui-ux-skill-prior-art.md`.
- **Output:** `docs/plans/2026-08-23-ui-ux-design-mcp-skill.md` (5/5 tasks); PR #39 +
  follow-up #40 merged to `main` (`624165c`) — the first time this repo's chain has
  gone plan → build → test → sweep → review → deliver → merge end to end.
- **Status:** Done
